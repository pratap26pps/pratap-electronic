...
export default function YourCart() {
  ...
  const subtotal = cart.reduce(
    (acc, item) =>
      acc + item.productId?.ProductPrice * (quantities[item._id] || 1),
    0
  );
  ...
  if (loading) {
    return (
      <div className="flex gap-1">
        ...
        <div className="text-center text-blue-900 mt-40 text-xl">
          Loading your cart...
        </div>
      </div>
    );
  ...
  if (user?.role === "owner") {
    return (
      <div className="text-center mt-40 text-lg font-medium text-red-600">
        Hey! You are an Owner, you cannot show public cart page{" "}
      </div>
    );
  }
