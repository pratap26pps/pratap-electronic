...
const PlaceOrder = ({ cartItems =[], customer, shippingFee = 45, gstRate = 0.18, coupon = null }) => {
  ...
  return (
    <div className="max-w-4xl mt-36 mx-auto p-6  rounded-xl shadow-md space-y-6">
      ...
      <p className="text-green-600 font-medium">
        Earn 3% reward on each order when you sign in. The amount will be added to your Evelta wallet after delivery!
      </p>
      <p className="text-orange-600">
        Free shipping on orders over ₹999.00. Add items worth ₹{Math.max(0, 999 - subtotal).toFixed(2)} to claim the discount!
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <p>{customer?.email}</p>
          <button className="text-blue-500 text-sm underline mt-2">Sign Out</button>
        </div>
      ...
      <div className="  p-4 rounded-md shadow-sm">
        ...
        <div className="border-t mt-2 pt-2 space-y-1">
          ...
          {coupon && <div className="flex justify-between text-green-600"><span>Discount</span><span>-₹{discount?.toFixed(2)}</span></div>}
          <div className="flex justify-between font-bold text-lg mt-2"><span>Total (INR)</span><span>₹{total?.toFixed(2)}</span></div>
        </div>
      </div>

      <button className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 px-6 rounded-lg shadow-md">
        Place Your Order
      </button>
    </div>
  );
